...
def get_experiment_name_replacement(system_name="Astrolabe"):
    return {"min latency": system_name, "min infass load": "INFaaS++",
            "request per seconds": "Instance-QPM"}

def get_scheduler_name_ordered(system_name="Astrolabe"):
    return ['Round Robin', 'random', 'INFaaS++', 'Instance-QPM', f'{system_name}-len-oracle', system_name]

# For backward compatibility
experiment_name_replacement = get_experiment_name_replacement()
scheduler_name_ordered = get_scheduler_name_ordered()
...
def main():
    ...
    global SYSTEM_NAME, experiment_name_replacement, scheduler_name_ordered
    SYSTEM_NAME = args.system_name
    experiment_name_replacement = get_experiment_name_replacement(SYSTEM_NAME)
    scheduler_name_ordered = get_scheduler_name_ordered(SYSTEM_NAME)
    ...
    for scheduler_name in os.listdir(data_dir):
        ...
        for directory in os.listdir(scheduler_dir):
            record = {"scheduler_name": "astrolabe"}
