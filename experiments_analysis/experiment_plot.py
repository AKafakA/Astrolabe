...
def get_experiment_name_replacement(system_name="Astrolabe"):
    return {"min new request latency": system_name, "min infass load": "INFaaS++",
            "request per seconds": "Min QPM",
            "min lunmnix load": "Llumnix-"}

def get_scheduler_to_color(system_name="Astrolabe"):
    return {
        'random': 'olive',
        'round robin': 'green',
        'Min QPM': 'purple',
        ...
        'Llumnix-': 'skyblue',
        system_name: 'black',
        f'{system_name}-len-oracle': 'red',
    }

# For backward compatibility
experiment_name_replacement = get_experiment_name_replacement()
scheduler_to_color = get_scheduler_to_color()
...
def plot_linear_with_different_qps(ax,
    ...
    if slo > 0 and len(capacity_indexes) > 0:
        ...
        selected_schedulers = ["Llumnix-", SYSTEM_NAME, f"{SYSTEM_NAME}-len-oracle"]
...
def plot_per_qps(experiments_set, output_dir,
    ...
    for qps in qps_set:
        ...
        for experiment in qps_experiments:
            ...
            for key in experiment_name_replacement.keys():
                ...
                if experiment_name == SYSTEM_NAME and not experiment["use_length_estimation"]:
                    experiment_name = f"{SYSTEM_NAME}-len-oracle"
    ...
    plot_linear_with_different_qps(axs[0, 1], average_ttft, "Average TTFT (s)",
                                   keep_legend=True, anchor=(3.7, 1.28),
                                   x_dim="QPS", keep_x_ticks_labels=True)
    ...
    if print_statis_table:
        ...
        baseline_schedulers = [k for k in index_names if k not in (SYSTEM_NAME, f"{SYSTEM_NAME}-len-oracle")]
        rows = []
        for qps in qps_set:
            for key, human, lower_is_better, metric_map in metrics_spec:
                ...
                # Fetch system / system-len-oracle values if present
                ...
                block_val = get_val(SYSTEM_NAME)
                block_star_val = get_val(f"{SYSTEM_NAME}-len-oracle")
                ...
                rows.append({
                    ...
                    SYSTEM_NAME: round(block_val, 4) if not math.isnan(block_val) else np.nan,
                    f"{SYSTEM_NAME} Δ": round(block_abs, 4) if not math.isnan(block_abs) else np.nan,
                    f"{SYSTEM_NAME} Δ%": round(block_pct, 2) if not math.isnan(block_pct) else np.nan,
                    f"{SYSTEM_NAME}-len-oracle": round(block_star_val, 4) if not math.isnan(block_star_val) else np.nan,
                    f"{SYSTEM_NAME}-len-oracle Δ": round(star_abs, 4) if not math.isnan(star_abs) else np.nan,
                    f"{SYSTEM_NAME}-len-oracle Δ%": round(star_pct, 2) if not math.isnan(star_pct) else np.nan,
                })
        ...
        if rows:
            ...
            # Print to console
            print(f"\n==== {SYSTEM_NAME} vs Best Baseline (per QPS) ====")
...
def main():
    ...
    parser.add_argument("--selected-schedulers", type=str, default="Astrolabe, Llumnix-, Astrolabe-len-oracle")
    ...
    # Update global mappings based on system name
    global SYSTEM_NAME, experiment_name_replacement, scheduler_to_color
    SYSTEM_NAME = args.system_name
    experiment_name_replacement = get_experiment_name_replacement(SYSTEM_NAME)
    scheduler_to_color = get_scheduler_to_color(SYSTEM_NAME)
    ...
    if args.plot_selected_scheduler_only:
        ...
        # Replace "Astrolabe" with actual system name if needed
        selected_schedulers = [SYSTEM_NAME if s == "Astrolabe" else s for s in selected_schedulers]
        selected_schedulers = [f"{SYSTEM_NAME}-len-oracle" if s == "Astrolabe-len-oracle" else s for s in selected_schedulers]
    else:
