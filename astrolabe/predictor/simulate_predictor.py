...
def calculate_total_unprocessed_tokens(response_data: dict, new_request_context_tokens: int,
    ...
    """
    Calculate total unprocessed tokens for Astrolabe-NoSim scheduling.
...
class SimulatePredictor(Predictor):
    ...
    def __init__(self, config, port, use_multiprocessing=False):
        ...
        # Astrolabe-NoSim (min_total_unprocessed_tokens) needs full request data like simulation-based metrics
    ...
    async def predict(self, target_request: Request):
        ...
        if self._need_to_predict:
            ...
            # Astrolabe-NoSim: Use length prediction without simulation
