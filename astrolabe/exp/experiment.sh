# This script is used to run benchmark and global scheduler with the Astrolabe framework.
...
LENGTH_ERROR_PCT=${36:-0}
LATENCY_ERROR_PCT=${37:-0}

# Burstiness parameter for gamma distribution (optional, default 1.0 = Poisson)
# Lower values = more bursty (e.g., 0.25, 0.5)
# Higher values = less bursty (e.g., 2.0)
BURSTINESS=${38:-1.0}

# Enable CPU tracking for predictor overhead analysis (optional, default false)
ENABLE_CPU_TRACKING=${39:-false}
...
if [ "$RESTART_VLLM" = "true" ]; then
  parallel-ssh --host $TARGET_HOST "cd Astrolabe && rm experiment_output/logs/*"
  sh astrolabe/exp/reset.sh
  if [ "$UPDATE_BLOCK_CODE" = "true" ]; then
    parallel-ssh -t 0 -h astrolabe/config/hosts "cd Astrolabe && git checkout $BRANCH_NAME"
    parallel-ssh -t 0 -h astrolabe/config/hosts "cd Astrolabe && git add -u . && git stash && git reset --hard HEAD~1 && git pull"
  fi
  sleep 60
  nohup sh astrolabe/exp/run_exp_vllm.sh $BATCH_CAP $MODEL $UPDATE_VLLM_CODE $VLLM_VERSION $MAX_MODEL_LENGTH $ENABLE_CHUNKED_PREFILL $BACKEND_WORKERS $MAX_NUM_BATCHED_TOKEN > /dev/null 2>&1 &
  sleep 60
  script_base="astrolabe/exp/run_exp_predictor"
  if [ "$PREDICTOR_WORKERS" -eq 1 ]; then
    ...
    for suffix in $suffix_range; do
      nohup sh "${script_base}_${suffix}.sh" $PREDICTOR_CONFIG_PATH $SCHEDULER_METRIC_TYPE $ENABLE_TIME_ESTIMATION $BATCH_CAP $ENABLE_CHUNKED_PREFILL $PREDICTOR_WORKERS $BRANCH_NAME $BATCH_SIZE_THRESHOLD_FOR_TIME_ESTIMATION $PREDICTOR_TIMEOUT_IN_SECONDS $ENABLE_CPU_TRACKING > /dev/null 2>&1 &
    done
    sleep 10
        ...
        for metric_type in $METRIC_TYPES; do
          ...
          for n in $N; do
              for use_estimation_len in $USE_ESTIMATION_LEN; do
                  ...
                  nohup sh astrolabe/exp/run_exp_global_scheduler.sh $TARGET_HOST $n $n $metric_type $HOST_CONFIG_PATH $GLOBAL_SCHEDULER_WORKERS $PREDICTOR_WORKERS $PROFILING_SAMPLE_RATE $TIMEOUT_IN_SECONDS $PREDICTOR_TIMEOUT_IN_SECONDS $AVAILABLE_INSTANCE $MAX_SLO $ENABLE_PREEMPTIVE_AUTO_PROVISIONING $LENGTH_ERROR_PCT $LATENCY_ERROR_PCT > /dev/null 2>&1 &
                  ...
                  if [ "$use_estimation_len" = "true" ]; then
                    parallel-ssh -i -t 0 --host $TARGET_HOST "cd Astrolabe && export PYTHONPATH=. && export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:/usr/local/lib/python3.10/dist-packages/nvidia/cudnn/lib:/usr/local/lib/python3.10/dist-packages/nvidia/nccl/lib:/usr/local/lib/python3.10/dist-packages/cusparselt/lib && python astrolabe/benchmark/benchmark_serving.py --ip_ports 127.0.0.1:8200 --tokenizer $MODEL --num_sampled_requests $num_queries --dataset_type $DATASET_TYPE --dataset_path $DATASET_PATH --qps $qps --backend block --log_filename $LOG_FILENAME --output_dir $OUTPUT_DIR  --data_start_index $START_INDEX --trust_remote_code --max_request_len $MAX_MODEL_LENGTH --timeout_in_seconds $TIMEOUT_IN_SECONDS --use_estimated_response_lens --distribution gamma --burstiness $BURSTINESS"
                  else
                    parallel-ssh -i -t 0 --host $TARGET_HOST "cd Astrolabe && export PYTHONPATH=. && export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:/usr/local/lib/python3.10/dist-packages/nvidia/cudnn/lib:/usr/local/lib/python3.10/dist-packages/nvidia/nccl/lib:/usr/local/lib/python3.10/dist-packages/cusparselt/lib && python astrolabe/benchmark/benchmark_serving.py --ip_ports 127.0.0.1:8200 --tokenizer $MODEL --num_sampled_requests $num_queries --dataset_type $DATASET_TYPE --dataset_path $DATASET_PATH --qps $qps --backend block --log_filename $LOG_FILENAME --output_dir $OUTPUT_DIR  --data_start_index $START_INDEX --trust_remote_code --max_request_len $MAX_MODEL_LENGTH --timeout_in_seconds $TIMEOUT_IN_SECONDS --distribution gamma --burstiness $BURSTINESS"
                  fi

                  sleep 10
                  parallel-ssh --host $TARGET_HOST "cd Astrolabe && mkdir experiment_output/$OUTPUT_DIR/running_logs"
